...
import { 
  ChevronLeft, Search, Settings, HelpCircle, 
  CreditCard, Users, LogOut, Plus, Pin, Brain,
...
export function Dock() {
  ...
  return (
    <div
      ref={dockRef}
      className={`fixed left-0 top-16 h-[calc(100vh-4rem)] transition-all duration-300 flex flex-col ${
    ...
    >
      ...
      <button 
        ...
        className="mx-4 mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors flex items-center gap-2 text-white"
      >
        <Plus size={20} />
      ...
      <div className="flex-1 overflow-y-auto mt-4">
        <ChatHistory 
          ...
          chats={chats}
          onSelectChat={(chat) => navigate(`/chat?id=${chat.id}`)}
          onDeleteChat={() => {}}
          onRenameChat={() => {}}
        />
      </div>
      ...
      <div className="border-t border-gray-200 dark:border-white/10 p-4 space-y-2">
        <Link
          to="/settings"
          className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 transition-colors text-gray-700 dark:text-gray-300"
        >
          <Settings size={20} />
        ...
        {user ? (
          <button
            ...
            className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors text-red-600 dark:text-red-400 w-full"
          >
            <LogOut size={20} />
          ...
          <Link
            to="/signin"
            className="flex items-center gap-3 px-4 py-2 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-colors text-blue-600 dark:text-blue-400"
          >
            <LogOut size={20} />
