...
import { 
  Sparkles, ArrowRight, Star, MessageSquare, 
  ThumbsUp, Calendar, User, ExternalLink 
} from 'lucide-react';
            ...
            <div className="space-y-4">
              {tools.map((tool) => (
                <Link
                  key={tool.id}
                  to={`/tools/${tool.id}`}
                  className="block p-4 bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition-all border border-gray-100 dark:border-gray-700"
                >
                  <div className="flex items-start gap-3">
                    {tool.logo_url ? (
                      <img src={tool.logo_url} alt={tool.name} className="w-12 h-12 rounded-lg" />
                    ) : (
