...
export async function getProfile(user: User): Promise<Profile | null> {
  ...
  try {
    ...
    if (!data) return await createDefaultProfile(user);
    return data;
  } catch (error) {
 ...
 */
export async function createDefaultProfile(user: User): Promise<Profile | null> {
  if (!user?.id) return null;
 ...
 */
export async function updateProfile(user: User, updates: Partial<Profile>): Promise<Profile | null> {
  if (!user?.id) return null;
 ...
 */
export async function uploadAvatar(user: User, file: File): Promise<string | null> {
  if (!user?.id) return null;
  ...
  try {
    ...
    if (dbError) throw new Error(`Failed to update avatar in database: ${dbError.message}`);
 ...
 */
export async function deleteAvatar(user: User): Promise<boolean> {
  if (!user?.id) {
 ...
 */
export async function enable2FA(user: User): Promise<{ secret: string; qrCode: string } | null> {
  if (!user?.id) {
 ...
 */
export async function verify2FA(user: User, token: string): Promise<boolean> {
  if (!user?.id) {
